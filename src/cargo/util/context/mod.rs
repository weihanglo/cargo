...
impl GlobalContext {
    ...
    fn get_path_base(&self, base_key: &str) -> CargoResult<Option<PathBuf>> {
        self.get::<Option<ConfigRelativePath>>(&base_key).map(|path_bases|
            path_bases.map(|path_bases| path_bases.resolve_path(self))
        )
    }
...
impl DependencyContext for GlobalContext {
    fn get_path_base_(&self, base_key: &str) -> CargoResult<Option<PathBuf>> {
...
pub trait Context: WorkspaceLoaderContext + ShellContext + UnstableFlagsContext + HomeContext + DependencyContext {}

impl Context for GlobalContext {}

pub mod prelude {
    pub use super::WorkspaceLoaderContext as _;
    pub use super::ShellContext as _;
    pub use super::UnstableFlagsContext as _;
    pub use super::HomeContext as _;
}
