...
use crate::resolver::dep_cache::RequirementError;
use crate::resolver::errors::{
    ActivateError, ResolveError, describe_path, no_candidates_error, version_conflict_error,
};
use crate::resolver::types::ConflictMap;
use crate::workspace::{Dependency, Registry};
...
fn native_error<T: Registry>(
    ...
    package_path: &[crate::workspace::PackageId],
) -> Option<ResolveError> {
...
fn unsatisfiable_dependency<T: Registry>(
    provider: &Provider<'_, T>,
    tree: &DerivationTree<PubGrubPackage, SemverPubgrub, UnavailableReason>,
) -> Option<(crate::workspace::PackageId, Dependency)> {
    match tree {
...
fn collect_members<T: Registry>(
    provider: &Provider<'_, T>,
    tree: &DerivationTree<PubGrubPackage, SemverPubgrub, UnavailableReason>,
    out: &mut Vec<crate::workspace::PackageId>,
) {
    match tree {
