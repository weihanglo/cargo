...
The `patches` field allows applying patches files to a dependency.
...
- **Registry sources**: Paths are relative to the package root of the `.crate` source tarball.
- **Git sources**: Paths are relative to the repository root.
  This matches the output of `git diff` and allows patches to work with workspace inheritance.

The current supported patch file formats are [Unified diff format] and [Git format patch].
These can be generated by `git diff`, `git format-patch`, or [GNU diffutils].

Note that these formats are loosely specified,
so support is best-effort.
