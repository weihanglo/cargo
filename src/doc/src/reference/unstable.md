...
* Tracking Issue: [#4648](https://github.com/rust-lang/cargo/issues/4648)

The `patch-files` feature allows applying unified diff patches to dependencies
in the `[patch]` section. This is useful for applying bug fixes or modifications
to dependencies without forking or maintaining a separate copy.
...
To use this feature via Cargo configuration, you need to use either
`-Zpatch-files` or the `[unstable]` table to enable it. For example,
...
*as part of [Overriding Dependencies](overriding-dependencies.md#the-patch-section)*

The `patches` field allows applying [unified diff format] patches to a dependency.
Patch files are applied in order after the package source is downloaded.
Patch file paths are relative to the manifest where `[patch]` is defined,
or [config-relative paths] when defined in Cargo configuration
like `.cargo/config.toml`.

[config-relative paths]: config.md#config-relative-paths

[unified diff format]: https://www.gnu.org/software/diffutils/manual/html_node/Unified-Format.html

```toml
[patch.crates-io]
foo = { version = "1.0", patches = ["patches/fix-bug.patch"] }
```

Patch files should be in unified diff format, such as those generated by
`git diff`, `git format-patch`, or [GNU diffutils].

[GNU diffutils]: https://www.gnu.org/software/diffutils/

> **Note:** The [unified diff format] is loosely specified, so support is
> best-effort and some edge cases may not be handled correctly.

A simple patch file example:
...
-    x * 2  // bug: should be x + 2
+    x + 2
 }
```

##### Patch path semantics

The paths inside patch files are interpreted differently depending on the source type:

- **Registry sources**: Paths are relative to the package root
  (the directory containing `Cargo.toml`).
- **Git sources**: Paths are relative to the repository root.
  This matches the output of `git diff` and allows patches to work
  with workspace inheritance.

##### Git source considerations

For git sources using workspace inheritance, all packages from the same
repository must use identical patch files. This is because Cargo copies the
entire repository (not just individual packages) to preserve workspace
relationships:

```toml
# OK: same patches for all packages from the same repo
[patch.crates-io]
bar = { git = "https://example.com/repo", patches = ["shared.patch"] }
baz = { git = "https://example.com/repo", patches = ["shared.patch"] }

# ERROR: different patches on same repo is not allowed
[patch.crates-io]
bar = { git = "https://example.com/repo", patches = ["bar.patch"] }
baz = { git = "https://example.com/repo", patches = ["baz.patch"] }
```
